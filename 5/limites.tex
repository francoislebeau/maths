...
Nous sentons déjà que nous allons nous faire traiter d'incompétents par vos profs particuliers,
...
\begin{howto}
    ...
    \begin{itemize}
        ...
        \item Si $f$ a une limite en $a$
            \begin{align}
                \lim_{x \to a} f(x) = b,
            \end{align}
            et si $g$ a une limite en $b$,
            alors
            \begin{align}
                \lim_{x \to a} g(f(x)) = \lim_{y \to b} g(y)
            \end{align}
            à condition que la limite à gauche ait un sens.
...
\begin{example}
    ...
    Puisque les limites latérales ne coïncident pas, nous écrivons
    \begin{align}
        \lim_{x \to 0} \frac 1 x & \text{ n'existe pas.}
    \end{align}
    Nous avons néanmoins une asymptote verticale $AV_0 \equiv x = 0$.
...
\begin{example}
    ...
    Puisque les limites latérales ne coïncident pas, nous écrivons
    \begin{align}
        \lim_{x \to 1} \frac{x^3 - 7x - 6}{x^2 - 3x +2} \text{ n'existe pas.}
    \end{align}
    Nous avons néanmoins une asymptote verticale $AV_1 \equiv x = 1$.
\end{example}
...
\begin{example}
    ...
    Nous avons donc un point creux en $(1, - \frac 5 2)$.
\end{example}
...
\begin{example}
    ...
    Puisque les limites latérales ne coïncident pas, nous écrivons
    \begin{align}
        \lim_{x \to 1} \frac {2x^2 + x -3}{x^2 - 2x +1} & \text{ n'existe pas.}
    \end{align}
    Nous avons néanmoins une asymptote verticale $AV_1 \equiv x = 1$.
\end{example}
...
\begin{example}
    ...
    \begin{align}
        ...
        &= -\frac 1 {2 \sqrt 2}.
    \end{align}
    Nous avons donc un point creux en $(0, -\frac 1 {2 \sqrt 2})$.
\end{example}
...
\begin{exercise}
    Définissez limite réelle en moins l'infini.
\end{exercise}
...
\begin{definition}
    ...
    Soit $f$ une fonction.
    Supposons que $\dom f$ ne soit pas majoré.
    On écrit
    \begin{align}
        \lim_{x \to +\infty} f(x) = \pm \infty
    \end{align}
    si \emph{$f(x)$ devient supérieur (resp. inférieur) à n'importe quel nombre réel}
    lorsque \emph{$x$ est suffisament grand avec $x \in \dom f$}.
\end{definition}

\begin{exercise}
    Définissez limite infinie en moins l'infini.
\end{exercise}
...
Toutes les limites infinies à l'infini ne donnent pas forcément lieu à une asymptote oblique.
...
\begin{example}
    ...
    Nous constatons cependant que le graphique de la fonction ne se rapproche d'aucune droite pour des valeurs de $x$ très grandes.
    \begin{plot}{0.25}{-12}{-2}{12}{12}
        \plotfunction{-12:12}{\x*\x/10}
    \end{plot}
\end{example}
...
\subsubsection{Limite de $\frac k {x^n}$, $n \in \N_0$}
...
\begin{example}
    ...
    \begin{align}
        ...
        &= \lim_{x \to \pm \infty} \frac {2x^2 + x^2 - 5} {3 x^2 - 5x + 2}\\
        &= \lim_{x \to \pm \infty} \frac {3 x^2 - 5} {3 x^2 - 5x + 2}\\
        &= \lim_{x \to \pm \infty} \frac {3 x^2} {3 x^2} = 1.\\
    \end{align}
    Nous avons donc une asymptote horizontale $AH \equiv y = 1$.
\end{example}
...
\begin{tikzpicture}
    ...
    child {%
        ...
        child {%
            ...
            edge from parent node [above] {\footnotesize{$a \notin \dom$}}
        }
        child {%
            node [bordered] {continue en $a$}
            edge from parent node [above] {\footnotesize{$a \in \dom$}}
        }
    };
    \draw [->,-latex] (B) to (F);
...
\begin{tikzpicture}
    ...
    child {%
        ...
        child {%
            node {$\begin{aligned}\lim_{x \to \pm \infty} k x^n\end{aligned}$}
            child {%
                node {$(\sign k) \infty$}
            ...
            child {%
                node {$(-\sign k) \infty$}
                edge from parent node [above] {\footnotesize{$n$ impair}}
                edge from parent node [below] {\footnotesize{$-\infty$}}
            }
        }
    };
